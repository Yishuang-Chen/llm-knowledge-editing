...
class Trace(contextlib.AbstractContextManager):
    ...
    def __init__(
        ...
        def retain_hook(m, inputs, kwargs=None, output=None):
            # PyTorch forward hooks have two signatures:
            # 1) hook(module, args, output)
            # 2) hook(module, args, kwargs, output) when with_kwargs=True
            # Normalize both to the same local variables.
            if output is None:
                output = kwargs
                kwargs = None
            if retain_input:
