...
def layer_stats(
    ...
    def resolve_local_dataset_path(name: str) -> Path | None:
        raw_path = Path(name)
        candidates = [raw_path]
        if not raw_path.is_absolute():
            project_root = Path(__file__).resolve().parents[4]
            easyedit_root = Path(__file__).resolve().parents[3]
            candidates.extend(
                [
                    project_root / raw_path,
                    easyedit_root / raw_path,
                ]
            )
    ...
    def get_ds():
        ds_path = resolve_local_dataset_path(ds_name)
        field = "text"

        if ds_path is not None:
            suffix = ds_path.suffix.lower()
            if suffix in {".json", ".jsonl"}:
                raw_ds = load_dataset("json", data_files=str(ds_path))
            elif suffix in {".txt", ".text"}:
                raw_ds = load_dataset("text", data_files=str(ds_path))
            else:
                raise ValueError(f"Unsupported local stats corpus format: {ds_path}")
        else:
            raw_ds = load_dataset(
                ds_name,
        ...
        if batch_tokens is not None and batch_tokens < maxlen:
            maxlen = batch_tokens
        return TokenizedDataset(raw_ds["train"], tokenizer, maxlen=maxlen, field=field)
